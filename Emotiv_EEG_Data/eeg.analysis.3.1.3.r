...
# Before using the eeg.analysis script, first ensure that your directory structure is correct. 
# You should create a seperate subject folder named with the subjects number (i.e. 28211) for each subject. 
# Each subject folder should contain the corresponding raw datas (i.e. 28211_ClosedPre1) file. All data files should be named # in the following way Subj#_[WhateverYouWant]1.txt. 
# If the number at the end of your file is a 1 your subject folder should only include one subject file, if the number is a 2 # your folder should #contain 2 files (i.e. 28211_ClosedPre1 and 28211_ClosedPre2) in this case only the second file (28211_ClosedPre2) will be analyzed.
...
analyze.folders <- function(session.prefix="pre", sampling=128) {
	for (d in temp_list){#dir()[file.info(dir())$isdir] ) {
		#filepath <- d  ir(d, full.names=T)[length(dir(d))]
		#filename <- dir(d, full.names=F)[length(dir(d))]
		#subject <- strsplit(filename, "_")[1]
		#ext <- strsplit(filename, "_")[2]
		#session <- strsplit(ext, ".t")[1]
		print(paste("d: ", d))
		session <- paste0(session.prefix, 1)# length(dir(d, full.names=T)[grep(session.prefix, dir(d, full.names=T))]), sep="")
		print(paste("sessoi: ", session))
